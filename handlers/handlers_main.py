...
def register_main_handlers(dp, bot, version):
    ...
    async def help_callback(call: CallbackQuery):
        ...
        """
        config = await get_valid_config(call.from_user.id)
        target_display = get_target_display(config, call.from_user.id)
        ...
        help_text = (
            ...
            "<b>♻️ Сбросить счётчик</b> — обнуляет количество уже купленных подарков, чтобы не создавать снова такую же конфигурацию.\n"
            "<b>💰 Пополнить</b> — депозит звёзд в бот.\n"
            "<b>↩️ Вывести</b> — возврат звёзд по ID транзакции или вывести все звёзды сразу.\n"
            ...
            "❗️ После изменения конфигурации, покупки автоматически не стартуют — включите 🟢 вручную.\n"
    ...
    async def reset_bought_callback(call: CallbackQuery):
        """
        Сброс счетчика купленных подарков и статуса выполнения.
        """
        config = await get_valid_config(call.from_user.id)
        config["BOUGHT"] = 0
        config["DONE"] = False
